...
buildah version
date >"$(hostname)"
until sudo docker run --rm -v "/usr/bin:/pwd" -w /tools --entrypoint /bin/sh "ghcr.io/labring-actions/cache:tools-amd64" -c "ls -lh && cp -a . /pwd" 2>/dev/null; do
...
kube_pkgs() {
  ...
  # cache binary
  for binary in kubectl kubelet kubeadm; do until curl -sLo "bin/$binary" "https://storage.googleapis.com/kubernetes-release/release/$vV/bin/linux/$ARCH/$binary"; do sleep 1; done; done
  # cache image
  if [[ amd64 == "$ARCH" ]]; then
    sudo cp -a bin/kubeadm /usr/bin/
  else
    until sudo curl -sLo /usr/bin/kubeadm "https://storage.googleapis.com/kubernetes-release/release/$vV/bin/linux/amd64/kubeadm"; do sleep 1; done
  fi
